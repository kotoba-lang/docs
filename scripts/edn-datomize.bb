...
;; kotoba-lang/docs 用に com-junkawasaki/root superproject の
;; manifest/edn-datomize.bb から移植（Phase 3 fanout。gftdcojp/net-kotobase の
;; Phase 2 pilot 版に wrap-generic モードが追加済みだったので、その版をベースに
;; 移植した）。schema-path をこの repo のルート schema.edn に向けている以外は
;; ロジック同一。
;;
...
;; 名前空間を付けた属性名にリネームする（wrap-map）。既に名前空間付きキーを
;; 持つ map は、既存の名前空間を保ったまま裸キーだけ default-ns を付与する
;; （wrap-generic）。値が Datomic の scalar valueType（string/long/double/
;; boolean/keyword、またはそれらの集合）に収まらないもの（入れ子 map、map を
;; 含む vector 等）は pr-str した文字列として保持する（valueType=string の
;; "blob" 属性にする — トップレベルの entity+attribute 粒度でのクエリは常に
;; 有効、blob の中身は呼び出し側で edn/read-string すれば読める）。属性定義は
;; repo ルートの schema.edn に自動登録する（Datomic/Datascript 両対応、
;; :db.install/_attribute 等の Datomic 固有キーは使わない）。
;;
;; 使い方:
;;   bb scripts/edn-datomize.bb wrap-map     <path> <ns>         — map 1個のファイルを変換
;;   bb scripts/edn-datomize.bb wrap-generic <path> <default-ns> — 既に名前空間付きキーの map を変換
